...
function FolderTreeNode({
  ...
  return (
    <div>
      <div
        className={`flex items-center gap-1 py-1 px-2 cursor-pointer rounded transition-all ${
          isDragOver
            ? 'bg-blue-600/30 ring-2 ring-blue-500 ring-inset'
            : isSelected
      ...
      >
        ...
        {/* Folder icon */}
        <svg
          className={`h-4 w-4 flex-shrink-0 ${isDragOver ? 'text-blue-400' : isSelected ? 'text-blue-400' : 'text-yellow-500'}`}
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
  ...
  return (
    <div className="py-2 relative">
      ...
      {contextMenu.visible && (
        <div
          ref={contextMenuRef}
          className="fixed z-50 bg-slate-800 border border-slate-700 rounded-lg shadow-xl py-1 min-w-48"
          style={{ left: contextMenu.x, top: contextMenu.y }}
        >
          <div className="px-3 py-2 border-b border-slate-700">
            <p className="text-xs text-slate-400 truncate">{contextMenu.folderPath === '/' ? 'Rot' : contextMenu.folderName}</p>
          </div>

      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-lg mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-900/30 rounded-lg">
              <DocumentDuplicateIcon className="h-6 w-6 text-amber-500" />
            </div>
        ...
        <div className="px-6 py-4 space-y-4">
          <p className="text-slate-300">
            Ett dokument med namnet <span className="font-medium text-white">{existingDocument.name}</span> finns
            redan i den här mappen. Vad vill du göra?
          </p>

          {/* Current document info */}
          <div className="p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
            <p className="text-sm text-slate-400">Befintligt dokument:</p>
            <div className="flex items-center justify-between mt-1">
              <p className="text-white font-medium">{existingDocument.name}</p>
          ...
          <div className="p-3 bg-blue-900/20 rounded-lg border border-blue-700/50">
            <p className="text-sm text-blue-400">Ny fil:</p>
            <p className="text-white font-medium mt-1">{newFile.name}</p>
        ...
        {/* Footer */}
        <div className="flex flex-col gap-3 px-6 py-4 border-t border-slate-700">
          {/* Primary action - Replace */}
          <button
            onClick={handleReplace}
            disabled={isReplacing}
            className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isReplacing ? (
              <>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onKeepBoth}
              disabled={isReplacing}
              className="flex-1 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
            ...
            <button
              onClick={onClose}
              disabled={isReplacing}
              className="flex-1 px-4 py-2 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition-colors disabled:opacity-50"
            >
              Avbryt
            </button>
          </div>

...
export default function MembersSettingsPage() {
  ...
  return (
    ...
    >
      {/* Header */}
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Medlemmar</h1>
        ...
        {canManage && (
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-medium hover:from-indigo-700 hover:to-purple-700 shadow-md shadow-indigo-500/20 transition-all"
          >
            <Plus className="w-4 h-4" />
            <span className="hidden sm:inline">Bjud in medlem</span>
            <span className="sm:hidden">Bjud in</span>
          </button>
        )}
      </motion.div>
        ...
        <div className="divide-y divide-slate-100">
          {members.map((member, index) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-medium shadow-sm flex-shrink-0">
                  ...
                  {/* Desktop: Role badge and delete button */}
                  <div className="hidden sm:flex items-center gap-2">
                    ...
                    {canManage ? (
                      <select
                        ...
                        className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[120px]"
                      >
                ...
                {/* Mobile: Actions row */}
                <div className="sm:hidden mt-3 flex items-center gap-2 pl-[52px]">
                  {canManage && (
                    <select
                      value={member.group_id || ''}
                      onChange={(e) => handleGroupChange(member.user_id, e.target.value)}
                      disabled={actionLoading === `group-${member.user_id}`}
                      className="flex-1 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="">Ingen grupp</option>
                      {groups.map((group) => (
                        <option key={group.id} value={group.id}>
          ...
          <div className="divide-y divide-slate-100">
            {invitations.map((invitation, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-slate-900 font-medium truncate text-sm sm:text-base">{invitation.email}</p>
                  <p className="text-slate-500 text-xs sm:text-sm">
                    Inbjuden {new Date(invitation.created_at).toLocaleDateString('sv-SE')}
                  </p>
                </div>
                <span className="hidden sm:inline-flex px-3 py-1.5 bg-slate-100 rounded-lg text-sm text-slate-600">
                  {getRoleDisplayName(invitation.role?.name as RoleName)}
                </span>
                <span className="sm:hidden px-2 py-1 bg-slate-100 rounded-lg text-xs text-slate-600">
                  {getRoleDisplayName(invitation.role?.name as RoleName)}
                </span>
                {canManage && (
                  <button
      ...
      {canDeleteProject(userRole) && (
        <motion.div
          variants={itemVariants}
          className="mt-8 sm:mt-12 bg-white/80 backdrop-blur-sm border border-red-200 rounded-2xl overflow-hidden shadow-sm"
        >
          <div className="p-3 sm:p-4 border-b border-red-100 bg-red-50/50 flex items-center gap-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  När du raderar ett projekt tas all data bort permanent.
                  <span className="hidden sm:inline"> Inklusive dokument, ärenden, checklistor och annan data.</span>
                  {' '}Denna åtgärd kan inte ångras.
                </p>
              </div>
              <button
                onClick={() => setShowDeleteConfirm(true)}
                className="w-full sm:w-auto px-4 py-2.5 bg-red-50 text-red-600 rounded-xl font-medium hover:bg-red-100 transition-colors whitespace-nowrap border border-red-200 text-center"
              >
                Radera projekt
              </button>
            </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-[100] sm:p-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white border border-slate-200 rounded-t-2xl sm:rounded-2xl max-w-md w-full p-4 sm:p-6 shadow-2xl"
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center flex-shrink-0">
                <AlertTriangle className="w-5 h-5 text-red-600" />
              </div>
              <h2 className="text-lg sm:text-xl font-bold text-slate-900">Radera projekt</h2>
            </div>
            <p className="text-sm sm:text-base text-slate-600 mb-4">
              Är du säker på att du vill radera <span className="text-slate-900 font-medium break-all">{project.name}</span>?
            ...
            <input
              ...
              className="w-full px-3 sm:px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500 mb-4 text-sm sm:text-base"
            />
            <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 sm:justify-end">
              <button
                ...
                className="px-4 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors border border-slate-200 sm:border-0"
              >
                Avbryt
              </button>
              <button
